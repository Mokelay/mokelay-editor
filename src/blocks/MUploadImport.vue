...
export const mUploadImportEditorTool = defineEditorTool<MUploadImportProps>({
  toolbox: {
    title: uploadImportTitle,
    icon: fieldIcon
  },
  propertyPanel: {
    get title() {
      return pageDslPropertyTitle(uploadImportTitle);
    },
    get fields(): EditorToolPropertyField[] {
      return [
        { key: 'mode', label: '模式', type: 'select', options: modeOptions },
        { key: 'accept', label: '文件类型', placeholder: '.xlsx,.xls,.csv' },
        { key: 'multiple', label: '允许多文件', type: 'checkbox' },
        { key: 'maxCount', label: '最多文件数' },
        { key: 'maxSizeMB', label: '单文件大小 MB' },
        { key: 'drag', label: '拖拽上传', type: 'checkbox' },
        { key: 'autoUpload', label: '选择后自动上传', type: 'checkbox' },
        { key: 'parsePreview', label: '本地预览', type: 'checkbox' },
        {
          key: 'template',
          label: '模板配置 JSON',
          type: 'textarea',
