...
const actionCardListTitle = '动作卡片列表';
...
export const mActionCardListEditorTool = defineEditorTool<MActionCardListProps>({
  ...
  propertyPanel: {
    get title() {
      return pageDslPropertyTitle(actionCardListTitle);
    },
    fields: [
      {
        key: 'items',
      ...
      { key: 'itemKey', label: '唯一标识路径', placeholder: 'key / uuid / id' },
      { key: 'titlePath', label: '标题路径', placeholder: 'title / name' },
      { key: 'descriptionPath', label: '描述路径', placeholder: 'description' },
      {
        key: 'variant',
        label: '样式',
        type: 'select',
  ...
  createInitialProps: () => ({
    items: cloneValue(defaultActionCardItems),
    ...actionCardListDefaults
  }),
